...
@section('shift.content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Crea turno</h3>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('shifts.calendar.store') }}">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="form-label">Data inizio</label>
                            <input type="text" name="startDate" class="form-control" value="{{ $startDate }}" required />
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="startHour" class="form-control" value="{{ $startHour }}" required />
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="endDate" class="form-control" value="{{ $endDate }}" required />
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="endHour" class="form-control" value="{{ $endHour }}" required />
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select type="text" id="users" name="users[]" class="form-control" multiple required>
                                @foreach(\App\User::shiftsAvailable() as $user)
                                    <option value="{{ $user->id }}" @if($user->id == request()->resource_id) selected @endif
                                    >{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
